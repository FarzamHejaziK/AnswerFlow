# Apple Code Signing & Notarization Report — AnswerFlow (macOS)
...
**Notary credential (local):** keychain profile `answerflow-notary` — verified via `xcrun notarytool history --keychain-profile answerflow-notary` (authenticates).
...
| notarytool profile | `answerflow-notary` works | `xcrun notarytool history --keychain-profile answerflow-notary` → "No submission history" (auth OK) |
...
**Modified**
- `electron-builder.signed.cjs` — **built-in** notarization (`mac.notarize: true`) via the `answerflow-notary` keychain profile; entitlements repointed to `build/`; `hardenedRuntime: true`; `gatekeeperAssess: false`; identity auto-discover.
...
## 5. AnswerFlow-specific capability review (requirement #5)
...
- App extracted from `AnswerFlow-2.6.0-arm64-mac.zip` → `codesign --verify --deep --strict` valid, `spctl` accepted / Notarized Developer ID, `stapler validate` worked. ✅
...
Decisions (confirmed with user): use the **`answerflow-notary` keychain profile** for credentials (re-enabled by unlocking the Mac), and produce a **styled DMG via `create-dmg`**.

Notary log confirmed the root cause precisely — submission `7b44d402…` (arm64 DMG) returned:
> `"statusSummary": "Archive contains critical validation errors"` · issue: **`"The signature of the binary is invalid"`** @ `AnswerFlow.app/Contents/MacOS/AnswerFlow`.
...
- Added `assets/dmg-background.png` (660×400 styled background) + volicon from `assets/answerflow.icns`.
...
`answerflow-notary` is no longer reachable by non-interactive `notarytool` calls (it worked during the build, then the session/keychain state changed — likely the Mac locked). I cannot fetch the Invalid notary log or re-notarize the DMG without it. **Resolve by EITHER:**
...
- (Quick) with your Mac unlocked, run `xcrun notarytool history --keychain-profile answerflow-notary` in an interactive terminal and click "Always Allow" if prompted, then tell me to retry.
...
2. **appId `com.electron.meeting-notes`** is a generic placeholder → migrate to e.g. `com.answerflow.app` (deliberate, announced migration: orphans existing TCC grants; auto-update keys on the feed so updates continue).
