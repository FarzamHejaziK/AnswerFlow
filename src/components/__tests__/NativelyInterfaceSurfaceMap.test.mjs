...
const source = readFileSync(join(repoRoot, 'src/components/NativelyInterface.tsx'), 'utf8');

test('NativelyInterface labels the visible live overlay surfaces', () => {
...
test('live transcription stays in the rolling transcript strip, not message rows', () => {
  ...
  assert.ok(rollingIndex > -1, 'rolling transcript strip must be rendered in NativelyInterface');
  assert.ok(aiRowsIndex > -1, 'AI answer rows must be rendered in NativelyInterface');
  assert.ok(
