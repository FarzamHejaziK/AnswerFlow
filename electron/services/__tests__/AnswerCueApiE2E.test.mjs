// electron/services/__tests__/AnswerCueApiE2E.test.mjs
//
// Env-gated real AnswerCue API smoke test. Enabled only when both:
//   RUN_ANSWERCUE_API_E2E=1
//   ANSWERCUE_API_KEY=<key>   (or ANSWERCUE_TRIAL_TOKEN=<token>)
//
...
const ENABLED = process.env.RUN_ANSWERCUE_API_E2E === '1' || process.env.RUN_ANSWERCUE_API_E2E === '1';
const KEY = process.env.ANSWERCUE_API_KEY ?? process.env.ANSWERCUE_API_KEY ?? '';
const TRIAL = process.env.ANSWERCUE_TRIAL_TOKEN ?? process.env.ANSWERCUE_TRIAL_TOKEN ?? '';
const API_BASE = process.env.ANSWERCUE_API_BASE ?? process.env.ANSWERCUE_API_BASE ?? 'https://api.natively.software';
...
describe('AnswerCue API real-network smoke', { skip: !ENABLED ? 'skip: set RUN_ANSWERCUE_API_E2E=1 with ANSWERCUE_API_KEY or ANSWERCUE_TRIAL_TOKEN to enable' : false }, () => {
  test('credentials are present in env (sanity check, key value not logged)', () => {
    const h = authHeader();
    assert.ok(h, 'ANSWERCUE_API_KEY or ANSWERCUE_TRIAL_TOKEN must be set when RUN_ANSWERCUE_API_E2E=1');
  });
