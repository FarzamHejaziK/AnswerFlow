 ...
 * AnswerCueProSTT
 *
 * Connects to the AnswerCue API WebSocket transcription endpoint.
 ...
 */
export class AnswerCueProSTT extends EventEmitter {
    ...
    public setSampleRate(rate: number): void {
        ...
        console.log(`[AnswerCueProSTT:${this.channel}] Sample rate ${previousRate}Hz → ${rate}Hz`);
        ...
        if (this.isActive && this.isConnected) {
            console.log(`[AnswerCueProSTT:${this.channel}] Rate changed mid-stream — reconnecting WS so server uses the new declared rate.`);
    ...
    public setRecognitionLanguage(key: string): void {
        ...
        if (key === 'auto') {
            ...
            console.log('[AnswerCueProSTT] Language set to auto-detect mode');
        } else {
            const config = RECOGNITION_LANGUAGES[key];
            if (!config) {
                console.warn(`[AnswerCueProSTT] Unknown language key: ${key}`);
                return;
            }
            ...
            this.languageAlternates = 'alternates' in config
                ...
                : [];
            console.log(`[AnswerCueProSTT] Language set: ${key} → ${this.languageBcp47}`,
        ...
        if (this.isActive && this.isConnected) {
            console.log('[AnswerCueProSTT] Language changed while active — reconnecting');
    ...
    /** No-op — AnswerCue API server handles VAD internally */
    public notifySpeechEnded(): void {}

    /** No-op — AnswerCue API server finalizes via VAD; no client-side flush available */
    ...
    public write(chunk: Buffer): void {
        ...
        if (!this.isConnected || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
            ...
            if (this.buffer.length > this.BUFFER_MAX_CHUNKS) {
                ...
                if (!this.bufferOverflowReported) {
                    ...
                    console.warn(`[AnswerCueProSTT:${this.channel}] Buffer overflow — dropping oldest chunks. Reconnect taking too long; transcript will have a gap.`);
            ...
            if (this.buffer.length <= 3 || this.buffer.length % 100 === 0) {
                ...
                console.log(`[AnswerCueProSTT:${this.channel}] Buffering chunk (buffer=${this.buffer.length}, isConnected=${this.isConnected}, ws=${wsState})`);
            }
            return;
        }
        ...
        if (this._chunksSent <= 5 || this._chunksSent % 200 === 0) {
            console.log(`[AnswerCueProSTT:${this.channel}] Sent chunk #${this._chunksSent} (${chunk.length}B) to server`);
        }
        this.ws.send(chunk);
    }
    ...
    private connect(_skipStagger = false): void {
        ...
        console.log(`[AnswerCueProSTT] Connecting (attempt ${this.reconnectAttempts + 1})...`);
        ...
        ws.on('message', (data: WebSocket.Data) => guard(() => {
            try {
                ...
                if (!msg.text || msg.is_final) {
                    console.log(`[AnswerCueProSTT:${this.channel}] Server msg`, {
                        type: msg.type,
                ...
                if (msg.error) {
                    console.error('[AnswerCueProSTT] Server error:', msg.error, msg.message || '');
                ...
                if (msg.status === 'connected') {
                    ...
                    console.log(`[AnswerCueProSTT] Connected via ${msg.provider}`);
                ...
                if (msg.language_detected) {
                    ...
                    console.log(`[AnswerCueProSTT] Auto-detected language: ${detected}`);
                ...
                console.error('[AnswerCueProSTT] Parse error:', err);
            }
        }));
        ...
        ws.on('error', (err: Error & { code?: string }) => guard(() => {
            ...
            if (this.isDnsFailure) {
                console.warn(`[AnswerCueProSTT:${this.channel}] DNS failure (${err.code}) — will retry in ${this.DNS_RETRY_MS / 1000}s without burning backoff`);
            } else {
                console.error('[AnswerCueProSTT] WebSocket error:', err.message);
            }
            this.isConnecting = false;
            this.isConnected  = false;
            this.emit('error', err);
        ...
        ws.on('close', (code: number) => guard(() => {
            ...
            console.log(`[AnswerCueProSTT] Connection closed (code ${code})`);
    ...
    private scheduleReconnect(): void {
        ...
        if (this.isDnsFailure) {
            ...
            console.warn(`[AnswerCueProSTT] DNS retry in ${this.DNS_RETRY_MS / 1000}s...`);
        ...
        console.log(`[AnswerCueProSTT:${this.channel}] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})...`);
    ...
    private flushBuffer(): void {
        ...
        if (this.bufferDroppedChunks > 0) {
            console.warn(`[AnswerCueProSTT:${this.channel}] Reconnected — flushing ${pending.length} buffered chunks; ${this.bufferDroppedChunks} were dropped during outage`);
        }
        this.bufferDroppedChunks = 0;
        this.bufferOverflowReported = false;
