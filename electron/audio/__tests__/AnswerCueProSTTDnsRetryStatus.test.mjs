...
test('AnswerCueProSTT DNS retries must not trip terminal STT failure counter', () => {
  ...
  assert.equal(
    ...
    false,
    'ENOTFOUND must remain retryable and never become a terminal STT failure while AnswerCueProSTT owns reconnect',
  );
...
test('AnswerCueProSTT temporary DNS failures must not trip terminal STT failure counter', () => {
  const stt = new EventEmitter();
  const statuses = [];
  wireSttErrorStatus(stt, (status) => statuses.push(status));
  ...
  assert.equal(
    statuses.some((status) => status.state === 'failed'),
    false,
    'EAI_AGAIN must remain retryable and never become a terminal STT failure while AnswerCueProSTT owns reconnect',
  );
  assert.equal(statuses.length, 5);
  assert.deepEqual([...new Set(statuses.map((status) => status.state))], ['reconnecting']);
  assert.deepEqual([...new Set(statuses.map((status) => status.reconnectAttempts))], [0]);
