...
//
// Strategy: load the compiled `AnswerCueProSTT.js`, then for each "session"
...
test('AnswerCueProSTT.stop() resets buffer-overflow latch and dropped-chunk counter so the next session can re-emit buffer-overflow', () => {
    const stt = new AnswerCueProSTT('fake-key', 'mic');
