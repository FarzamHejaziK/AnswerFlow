...
//
// Net effect of the bug: starting a meeting opens two AnswerCueProSTT
...
// loudly.
//
// Strategy: load the COMPILED AnswerCueProSTT with `Module._load` patched so
...
const { AnswerCueProSTT } = await import(path.join(distRoot, 'AnswerCueProSTT.js'));
...
test('connect() must NOT stagger same-apiKey connections (mic + system concurrent)', async () => {
    ...
    const sysStt = new AnswerCueProSTT(API_KEY, 'system');
    const micStt = new AnswerCueProSTT(API_KEY, 'mic');
...
test('language_detected reconnect must fire at ~250 ms (no stagger added on top)', async () => {
    ...
    const stt = new AnswerCueProSTT('lang-detected-key', 'mic');
    ...
    // handler body from AnswerCueProSTT.ts (language_detected path).
...
test('AnswerCueProSTT must not expose a per-key stagger map (structural guard)', async () => {
    ...
    assert.equal(
        AnswerCueProSTT.nextSlotByKey,
        undefined,
        'AnswerCueProSTT.nextSlotByKey must not exist — the per-key stagger was deliberately removed (Deepgram concurrency is per-project quota, not per-key serial)',
    );
    assert.equal(
        AnswerCueProSTT.SLOT_INTERVAL_MS,
        undefined,
        'AnswerCueProSTT.SLOT_INTERVAL_MS must not exist — the 3000 ms stagger interval was deliberately removed',
    );
});
